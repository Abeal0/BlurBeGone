...
class ImageDisplay(QWidget):
    ...
    def start_processing(self):
        ...
        for label in [
            ...
            label.setMaximumSize(
                512, 384
            )  # Set a maximum size for each label to prevent overflow

        # Center the original image label by spanning both columns
        self.grid_layout.addWidget(self.original_label, 1, 0, 1, 2)  # Span both columns
        self.grid_layout.addWidget(self.enhanced_label, 2, 0)
        self.grid_layout.addWidget(self.edges_label, 2, 1)
        self.grid_layout.addWidget(
            self.annotated_label, 3, 0, 1, 2
        )  # Span both columns for YOLO detection
    ...
    def resize_labels(self):
        """Resize image labels to fit within the main window dimensions."""
        for label in [
            self.original_label,
            self.enhanced_label,
            self.edges_label,
