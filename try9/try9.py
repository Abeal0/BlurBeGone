...
def enhance_color(image, alpha=1.5):
    """Enhance the color by increasing saturation."""
    hsv_image = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)

    # Increase the saturation
    h, s, v = cv2.split(hsv_image)
    s = cv2.multiply(s, alpha)  # Increase saturation
    s = np.clip(s, 0, 255)  # Ensure saturation stays within bounds
...
def dehaze_and_enhance(image_path, output_path):
    ...
    # Step 4: Enhance color on the recovered image
    color_enhanced_image = enhance_color(recovered_image)

    # Step 5: Apply CLAHE to the color-enhanced image
    clahe_image = clahe(color_enhanced_image)

    # Step 6: Sharpen the CLAHE image
...
# Example usage
dehaze_and_enhance(
    "C:/Users/hassa/OneDrive - Umich/Desktop/2024 HACKATHON/HackDearborn24/try9/foggyimage_person.jpg",
